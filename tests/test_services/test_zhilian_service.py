...
@pytest.mark.asyncio
async def test_zhilian_service_crud():
...
@pytest.mark.asyncio
async def test_zhilian_get_job_list():
    async with async_session_maker() as session:
        service = ZhilianService(session)
        await service.insert_job({"job_id": "1", "job_title": "A", "company_name": "C1", "delivered": 1})
        await service.insert_job({"job_id": "2", "job_title": "B", "company_name": "C2", "delivered": 0})
