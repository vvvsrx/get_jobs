...
export default function LiepinPage() {
  ...
  const fetchAllData = async () => {
    try {
      const response = await fetch('/api/liepin/config')
      ...
      if (data.config) {
        ...
        // 兼容已有数据：如果数据库中存的是城市名称，转换为 code
        if (data.options?.city && normalized.city) {
          const matched = data.options.city.find((c: LiepinOption) => c.code === normalized.city || c.name === normalized.city)
          if (matched) {
            normalized.city = matched.code
          }
        }
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.ok) {
        // 统一保存 Cookie（Liepin）
        try {
          await fetch('/api/cookie/save?platform=liepin', { method: 'POST' })
        } catch (e) {
  ...
  const handleLogin = async () => {
    try {
      setIsLoggingIn(true)
      const response = await fetch('/api/liepin/login', {
        method: 'POST',
  ...
  const handleStartDelivery = async () => {
    try {
      ...
      const response = await fetch('/api/liepin/start', {
        method: 'POST',
      })
      const data = await response.json()
  ...
  const handleStopDelivery = async () => {
    try {
      const response = await fetch('/api/liepin/stop', {
        method: 'POST',
      })
      const data = await response.json()
  ...
  const triggerLogout = async () => {
    try {
      const response = await fetch('/api/liepin/logout', { method: 'POST' })
      const data = await response.json()
      if (data.success) {
        setIsLoggedIn(false)
        setIsDelivering(false)
        ...
        actions={
          <div className="flex items-center gap-2">
            {checkingLogin ? (
              ...
              <Button size="sm" disabled className="rounded-full bg-amber-400 text-white cursor-not-allowed px-4 shadow animate-pulse">
                <BiPlay className="mr-1" /> 正在等待扫码...
              </Button>
            ) : !isLoggedIn ? (
              <Button
                onClick={handleLogin}
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2 md:col-span-2">
                <Label htmlFor="greeting">打招呼语</Label>
                <Input
                  id="greeting"
                  value={config.greeting || ''}
                  onChange={(e) => setConfig({ ...config, greeting: e.target.value })}
                  placeholder="您好，我对这个职位很感兴趣，希望能有机会进一步沟通。"
                />
                <p className="text-xs text-muted-foreground">点击"聊一聊"时自动发送的招呼语</p>
              </div>
            </div>
          </CardContent>
        </Card>
