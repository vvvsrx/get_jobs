...
import { BiSave, BiKey, BiLinkExternal, BiCodeAlt, BiInfoCircle, BiRefresh } from 'react-icons/bi'
...
export default function EnvConfig() {
  ...
  const handleImportOptions = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('更新基础数据失败')
      }

      const result = await response.json()

      if (result.success) {
        setImportResult({ success: true, message: result.message || '更新成功' })
      } else {
        setImportResult({ success: false, message: result.message || '更新失败' })
      }
      setShowImportDialog(true)
    } catch (error) {
      console.error('更新基础数据失败:', error)
      setImportResult({ success: false, message: '更新基础数据失败：网络或服务异常。' })
      setShowImportDialog(true)
    } finally {
      setImporting(false)
    }
        ...
        actions={
          <div className="flex gap-2">
            <Button
              ...
              disabled={importing}
              size="sm"
              className="rounded-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-4 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <BiRefresh className={`mr-1 ${importing ? 'animate-spin' : ''}`} /> {importing ? '更新中...' : '更新基础数据'}
            </Button>
            <Button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" role="dialog" aria-modal="true">
            <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl w-[92%] max-w-sm border border-gray-200 dark:border-neutral-800 animate-in fade-in zoom-in-95">
              <Card className="border-0">
                <CardHeader className="pb-2">
                  <CardTitle className="text-lg flex items-center gap-2">
                    <BiRefresh className={importResult.success ? 'text-green-500' : 'text-red-500'} />
                    {importResult.success ? '更新成功' : '更新失败'}
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-0">
                  <p className="text-sm text-muted-foreground mb-4">{importResult.message}</p>
                  <div className="flex justify-end gap-2">
                    <Button
                      onClick={() => setShowImportDialog(false)}
                      className={`rounded-full px-4 ${importResult.success ? 'bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white' : 'bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white'}`}
                    >
                      知道了
                    </Button>
                  </div>
